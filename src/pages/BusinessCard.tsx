  ...
  return <main className="business-card-page">
    ...
    <article className="business-card">
      ...
      <div className="business-card-content">
        ...
        {activeTab === "services" && <div className="business-card-services">{offers.map((offer) => <a key={offer.id} className="business-card-service" href={offer.href} target="_blank" rel="noreferrer"><span><strong>{offer.label}</strong><small>{offer.price} · {offer.description}</small></span><ArrowUpRight /></a>)}{serviceLinks.map(([label, description]) => <button type="button" key={label} className="business-card-service" onClick={() => openService(label)}><span><strong>{label}</strong><small>{description}</small></span><ArrowUpRight /></button>)}<a className="business-card-service" href="/checklist?utm_source=carte" onClick={() => trackConversionEvent("tool_click", "business_card_checklist", "/checklist")}><span><strong>CHECKLIST AUDIT — GRATUITE</strong><small>0 € · La checklist en 7 axes + les prompts IA pour auditer votre site vous-même.</small></span><ArrowUpRight /></a></div>}
