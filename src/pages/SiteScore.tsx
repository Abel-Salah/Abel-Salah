...
type AuditPage = { url: string; screenshot?: string | null; title?: string | null };
type AuditReport = { overall: number; criteria: AuditCriterion[]; priorities: string[]; limitations: string[]; screenshot?: string | null; pages?: AuditPage[]; pagesAnalyzed?: number };
...
const SiteScore = () => {
  ...
  const scanSite = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setScanReport({ ...data.audit, screenshot: data.screenshot ?? null, pages: data.pages ?? [], pagesAnalyzed: data.pagesAnalyzed ?? data.pages?.length ?? 0 });
      setLoadingStep(3);
  ...
  return (
    <main className="min-h-screen bg-background px-4 pb-24 pt-32 text-foreground md:px-8">
      ...
      <section className="mx-auto max-w-6xl">
        ...
        <form onSubmit={scanSite} className={`mt-10 max-w-3xl rounded-2xl border border-primary/25 bg-primary/5 p-6 ${scanReport ? "print:hidden" : ""}`}>
          ...
          <p className="mt-3 text-xs text-muted-foreground">Entrez simplement votre domaine, avec ou sans https://. L’audit automatique analyse jusqu’à 5 pages publiques : contenu, structure, SEO, conversion et crédibilité, avec captures pleine page. Aucun mot de passe n’est demandé.</p>
          ...
          {scanReport && <div className="mt-6 rounded-2xl border border-border bg-background p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="label-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Rapport automatique · {scanReport.pagesAnalyzed ?? scanReport.pages?.length ?? 1} page(s)</p><p className="mt-1 text-2xl font-bold text-primary">{scanReport.overall * 10}/100</p></div><button type="button" onClick={() => window.print()} className="inline-flex items-center gap-2 rounded-full border border-border px-4 py-2 text-sm font-semibold"><Download className="h-4 w-4" /> Télécharger / imprimer en PDF</button></div><div className="mt-5 grid gap-3 sm:grid-cols-2">{scanReport.criteria.map((criterion) => <div key={criterion.key} className="rounded-xl border border-border p-3"><div className="flex items-center justify-between gap-3"><p className="font-semibold">{criterion.label}</p><strong className="text-primary">{criterion.score}/10</strong></div><p className="mt-2 text-sm text-muted-foreground">{criterion.evidence?.[0] ?? "Aucun élément vérifiable détecté."}</p></div>)}</div></div>}
        </form>

        {scanReport && <section className="mt-10 print:mt-0 print:block"><header className="mb-8 hidden print:block"><p className="label-mono text-xs uppercase tracking-[0.2em]">Abel SALAH · Audit express du site</p><h2 className="mt-2 text-3xl font-bold">Rapport d’analyse — {scannedHost}</h2><p className="mt-2 text-sm">Généré le {new Date().toLocaleDateString("fr-FR")} · {scanReport.pagesAnalyzed ?? scanReport.pages?.length ?? 1} page(s) analysée(s)</p></header><div className="grid gap-8 lg:grid-cols-[1.15fr_0.85fr]"><div className="space-y-6"><div className="rounded-2xl border border-border p-6 print:break-inside-avoid"><h2 className="text-2xl font-bold">Ce qu’il faut corriger en priorité</h2><ol className="mt-5 space-y-3">{scanReport.priorities.map((priority, index) => <li key={`${priority}-${index}`} className="flex gap-3"><span className="font-bold text-primary">{index + 1}.</span><span>{priority}</span></li>)}</ol></div><div className="rounded-2xl border border-border p-6 print:break-inside-avoid"><h2 className="text-2xl font-bold">Plan d’action détaillé</h2><div className="mt-5 space-y-6">{scanReport.criteria.map((criterion) => <article key={criterion.key} className="print:break-inside-avoid"><h3 className="font-bold">{criterion.label} · {criterion.score}/10</h3><ul className="mt-2 list-disc space-y-1 pl-5 text-sm text-muted-foreground">{criterion.recommendations.map((recommendation, index) => <li key={`${recommendation}-${index}`}>{recommendation}</li>)}</ul></article>)}</div></div>{scanReport.limitations.length > 0 && <p className="text-sm text-muted-foreground print:break-inside-avoid"><strong>Limites :</strong> {scanReport.limitations.join(" ")}</p>}</div>{(scanReport.pages?.length ? scanReport.pages : scanReport.screenshot ? [{ url: url || scannedHost, screenshot: scanReport.screenshot }] : []).length > 0 && <div className="space-y-4"><h2 className="text-2xl font-bold">Captures des pages analysées</h2>{(scanReport.pages?.length ? scanReport.pages : [{ url: url || scannedHost, screenshot: scanReport.screenshot }]).map((page, index) => page.screenshot && <figure key={`${page.url}-${index}`} className="rounded-2xl border border-border p-4 print:break-inside-avoid"><figcaption className="mb-3 text-sm font-semibold">Page {index + 1} · {page.url}</figcaption><img src={page.screenshot} alt={`Capture de la page ${page.url}`} className="w-full rounded-xl border border-border" /></figure>)}</div>}</div></section>}

        <div className="mt-16 grid gap-12 lg:grid-cols-[1fr_0.7fr] lg:items-start">
          <div className={`space-y-8 border-y border-border py-8 ${scanReport ? "print:hidden" : ""}`}>
          ...
          <aside className={`rounded-2xl border border-primary/30 bg-primary/5 p-8 lg:sticky lg:top-28 ${scanReport ? "print:hidden" : ""}`}>
            <Gauge className="h-8 w-8 text-primary" />
            <p className="mt-8 label-mono text-xs uppercase tracking-[0.2em] text-muted-foreground">Score indicatif</p>
